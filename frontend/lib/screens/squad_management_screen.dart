/// Halaman Manajemen Skuad & Status Pemain STEMA
/// 
/// Halaman ini berfungsi bagi pelatih untuk mengatur status pemain sebelum match (Main, Cadangan, Pemanasan, Cedera, Tidak Hadir),
/// melakukan substitusi pemain masuk/keluar saat pertandingan berlangsung, mengedit level stamina,
/// serta memperbarui statistik individu (gol & assist) real-time yang disinkronkan ke server backend.
...
/// Widget Halaman Skuad: SquadManagementScreen
...
class _SquadManagementScreenState extends State<SquadManagementScreen> {
  // ── State Manajemen Skuad ──
  List<Map<String, dynamic>> _players = []; // Menyimpan daftar seluruh data pemain dari server Node.js
  bool _isLoading = true;                  // Penanda status loading saat mengambil data dari database

  @override
  void initState() {
    super.initState();
    // 1. Memastikan koneksi socket aktif dan meminta data roster
    SocketService().connect();
    SocketService().socket.emit('request_sync');

    // 2. Menerima sinkronisasi data pemain secara real-time via event 'stamina_sync'
  ...
  /// Mengirimkan pembaruan status pemain (Main/Cadangan/Pemanasan/dll) ke server Node.js
  /// Menggunakan event 'edit_player' dengan parameter payload model lengkap
  void _updateStatus(String playerName, String newStatus) {
    final player = _players.firstWhere((p) => p['nama'] == playerName);
    final payload = {
      ...player,
      'originalName': player['nama'], // originalName digunakan sebagai key pencarian unik di database backend
      'status': newStatus,
    };
    SocketService().socket.emit('edit_player', payload);
  }

  /// Mengirimkan pembaruan stamina pemain hasil koreksi manual pelatih ke server Node.js
  /// Menggunakan event 'edit_player' dengan parameter payload model lengkap
              ...
              actions: [
                ...
                ElevatedButton(
                  ...
                  onPressed: (selectedOut == null || selectedIn == null) ? null : () {
                    // Eksekusi: Masuk jadi Main, Keluar jadi Cadangan/Cedera/Kartu Merah
                    ...
                    // Menentukan status akhir pemain keluar berdasarkan alasan substitusi
                    ...
                    // Melakukan update status pemain di server database
                    _updateStatus(selectedOut!, finalOutStatus);
                    _updateStatus(selectedIn!, 'Main');
                    
                    // Mengirimkan event khusus 'log_substitution' ke server backend Node.js untuk pencatatan histori pertandingan
