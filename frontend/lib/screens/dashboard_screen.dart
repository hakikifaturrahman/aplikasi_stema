/// Halaman Dashboard Utama Aplikasi STEMA (Stamina & Match Analytics)
/// 
/// Halaman ini berfungsi sebagai pusat kontrol pertandingan live, visualisasi ringkasan tim,
/// rekomendasi keputusan taktis dari Rule Engine, dan navigasi menu cepat (Quick Menu).
/// Berkomunikasi secara real-time dengan server Node.js melalui Socket.IO.
...
/// Widget Halaman Utama: DashboardScreen
/// Menampilkan ringkasan status skuad, panel kontrol pertandingan langsung (Live Match Panel),
/// input manual statistik, dan rekomendasi performa/stamina pemain secara real-time.
...
class _DashboardScreenState extends State<DashboardScreen> {
  // ── State Simulasi Pertandingan Langsung (Live Match Simulation) ──
  int _skor1 = 0;         // Skor gol untuk tim kita (Real Madrid)
  int _skor2 = 0;         // Skor gol untuk tim lawan
  int _menit = 0;         // Waktu pertandingan berjalan dalam menit
  bool _isLive = false;   // Status penanda apakah pertandingan sedang aktif berjalan
  Timer? _timer;          // Objek Timer berkala untuk pembaruan menit & pengiriman status secara real-time

  // ── State Ekstra Waktu, Penalti & Fase Pertandingan ──
  String _fase = 'Persiapan'; // Menyimpan fase match saat ini (Persiapan, Babak 1, Injury 45, Half Time, Babak 2, dll)
  int _tambahanWaktu = 0;     // Jumlah tambahan waktu (injury time) yang ditentukan oleh wasit
  int _menitLebih = 0;        // Menit berjalan pada masa tambahan waktu (injury time)
  int _penalti1 = 0;          // Skor penalti tim kita
  int _penalti2 = 0;          // Skor penalti tim lawan
  final TextEditingController _injuryCtrl = TextEditingController(); // Controller teks untuk dialog input injury time

  // ── State Statistik Live Match ──
  int _shots = 0;             // Total tembakan melesat
  int _shotsOnTarget = 0;     // Total tembakan ke arah gawang (on target)
  final TextEditingController _possessionCtrl = TextEditingController(); // Controller teks input ball possession
  final TextEditingController _passesCtrl = TextEditingController();     // Controller teks input total operan
  ...
  // Getters Helper untuk mengolah ringkasan status pemain
  
  /// Mengambil jumlah pemain dengan status 'Main' (sedang berada di lapangan)
  int get _aktif => _players.where((p) => p['status'] == 'Main').length;

  /// Mengambil jumlah pemain yang cedera atau tidak hadir
  int get _cedera => _players
      .where((p) => p['status'] == 'Cedera' || p['status'] == 'Tidak Hadir')
      .length;

  /// Menghitung rata-rata stamina pemain aktif (Main & Pemanasan)
  ...
  /// Nilai rata-rata rating performa tim (nilai default statis)
  double get _avgRating => 7.4;

  /// Mengambil daftar pemain aktif atau pemanasan yang memiliki stamina di bawah 40% (kondisi kritis)
  ...
  /// Mendapatkan teks status keunggulan skor pertandingan saat ini
  ...
  /// Mendapatkan warna penanda status pertandingan saat ini (Hijau = Menang, Oranye = Seri, Merah = Kalah)
  ...
  void initState() {
    super.initState();
    // Memulai pemantauan menit secara berkala (timer simulasi pertandingan)
    _startLiveTimer();
    // Menginisialisasi pendengar event Socket.IO
    _setupSocketListeners();
    // Mengirim permintaan sinkronisasi awal ('request_sync') ke server agar data ter-update
    SocketService().socket.emit('request_sync');
  }

  /// Menyiapkan listener Socket.IO untuk event sinkronisasi real-time
  void _setupSocketListeners() {
    // Memastikan koneksi socket aktif
    SocketService().connect();

    // Membersihkan listener 'live_match_sync' lama dan mendaftarkan yang baru untuk sinkronisasi skor & waktu
    ...
    // Membersihkan listener 'stamina_sync' lama dan mendaftarkan yang baru untuk sinkronisasi daftar stamina pemain
    SocketService().socket.off('stamina_sync');
    SocketService().socket.on('stamina_sync', (data) {
      if (mounted && data != null) {
        setState(() {
          // Memetakan ulang list data mentah dari server ke state local `_players`
  ...
  /// Memulai Timer Simulasi Pertandingan Live
  /// Timer akan berjalan setiap 4 detik (merepresentasikan 1 menit durasi di game sebenarnya).
  /// Ketika pertandingan berakhir di babak tertentu, timer akan memicu dialog presentasi laporan stamina.
  void _startLiveTimer() {
    _timer = Timer.periodic(const Duration(seconds: 4), (t) {
      if (!_isLive) return; // Abaikan jika pertandingan sedang dihentikan / dijeda
      ...
      setState(() {
        if (_fase == 'Babak 1') {
          if (_menit < 45) {
            ...
            _isLive = false;
            _fase = 'Tunggu Injury 45'; // Menunggu wasit menginput injury time babak pertama
          }
          ...
          if (_menitLebih < _tambahanWaktu) {
            ...
            _isLive = false;
            _fase = 'Half Time';
            _showStaminaPresentationDialog(); // Tampilkan rekap stamina saat jeda babak
          }
          ...
          if (_menit < 90) {
            ...
            _isLive = false;
            _fase = 'Tunggu Injury 90'; // Menunggu wasit menginput injury time babak kedua
          }
          ...
          if (_menitLebih < _tambahanWaktu) {
            ...
            _isLive = false;
            if (_skor1 == _skor2) {
              _fase = 'Tunggu Extra Time'; // Jika skor seri, lanjut ke babak perpanjangan waktu
            } else {
              _fase = 'Full Time';
            }
          ...
          if (_menitLebih < _tambahanWaktu) {
            ...
            _isLive = false;
            if (_skor1 == _skor2) {
              _fase = 'Penalti'; // Jika masih seri setelah Extra Time, lanjut adu penalti
            } else {
              _fase = 'Full Time';
            }
            _showStaminaPresentationDialog();
      ...
      // Mengirim status terbaru pertandingan ke server backend agar disinkronkan ke panel admin/pengamat lain
  ...
  /// Menampilkan dialog input tambahan waktu (Injury Time) yang diinstruksikan oleh wasit
  void _inputInjury(String nextPhase) {
    _injuryCtrl.text = '3'; // Nilai default tambahan waktu adalah 3 menit
    showDialog(
      context: context,
  ...
  @override
  void dispose() {
    // Membatalkan Timer agar tidak menyebabkan kebocoran memori saat screen dihancurkan
    _timer?.cancel();
  ...
  /// Menampilkan lembar bawah (Bottom Sheet) yang berisi riwayat notifikasi peringatan kritis dari Rule Engine
  void _showNotificationPanel() {
    setState(() {
      globalHasUnreadNotification = false; // Reset tanda notifikasi belum dibaca
    });
                ...
                child: globalNotifications.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          // Memformat pencetakan waktu notifikasi
  ...
  // ── Presentasi Laporan Stamina Pemain Setelah Istirahat (Break/Half-Time/Full-Time) ──
  /// Menampilkan status ringkasan fisik pemain utama vs cadangan dalam bentuk persentase progress bar.
  ...
  /// Membangun tampilan daftar grup stamina (Pemain Utama vs Cadangan)
  ...
  // ── Rekomendasi dinamis dari Rule Engine lokal di sisi Client ──
  /// Menghasilkan rekomendasi substitusi atau perubahan taktis berdasarkan ambang batas stamina pemain aktif.
  List<Map<String, dynamic>> get _rekomendasi {
    ...
    // Rule tambahan: jika waktu babak kedua sudah lewat menit 60 dan tim dalam kondisi tertinggal skor
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // ── A. Seksi Ringkasan Tim ──
              _buildSectionLabel('A. Ringkasan Tim', 'Season 25/26'),
              const SizedBox(height: 14),
              _buildTeamSummary(),
              const SizedBox(height: 24),

              // ── B. Seksi Kontrol Pertandingan Live ──
              _buildSectionLabel(
                'B. Live Match Panel',
              ...
              // ── Seksi Menu Pintasan Navigasi (Quick Menu) ──
  ...
  /// Membangun Header Atas Dashboard CF Real Madrid
  Widget _buildHeader() {
    return Builder(
      builder: (context) {
        return Row(
