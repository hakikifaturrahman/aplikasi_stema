...
/// Widget Screen: PertandinganScreen
/// Halaman manajemen pertandingan (Match Management) untuk merekam formasi awal (Starting XI),
/// skor akhir, hasil akhir, serta catatan taktis pelatih untuk disimpan ke dalam database.
...
class _PertandinganScreenState extends State<PertandinganScreen> {
  // GlobalKey untuk penanganan validasi status Form
  final _formKey = GlobalKey<FormState>();

  // State pilihan Dropdown default
  String _kompetisi = 'Liga Domestik';
  String _formasi = '4-3-3';
  String _hasil = 'Menang (W)';
  DateTime? _selectedDate; // Tanggal tanding terpilih

  // Controller isian formulir
  ...
  List<dynamic> _players = [];       // Menyimpan daftar pemain lengkap dari server
  Map<String, bool> _startingXI = {}; // Melacak pilihan checkbox Starting XI tiap pemain

  // Getter menghitung total pemain yang terpilih sebagai Starting XI
  ...
  // Melakukan inisialisasi koneksi Socket dan sinkronisasi data roster pemain
  void _connectToSocket() {
    SocketService().connect();
    SocketService().socket.emit('request_sync');

    // Mendengarkan event 'stamina_sync' untuk mengambil data pemain aktif
    SocketService().socket.on('stamina_sync', (data) {
      if (mounted) {
        setState(() {
          _players = data;
          // Menginisialisasi map checkbox starting XI pemain agar bernilai false di awal
  ...
  // Menampilkan dialog pemilihan tanggal tanding
  ...
  Widget build(BuildContext context) {
    bool isLoading = _players.isEmpty; // Menandai jika data roster masih kosong/memuat
      ...
      body: isLoading 
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              // Bagian A: Info Detil Pertandingan
              ...
              // Bagian B: Pemilihan Starting XI (Maks 11 Pemain)
              Row(
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return CheckboxListTile(
                      ...
                      onChanged: (bool? val) {
                        setState(() {
                          // Validasi agar starting XI tidak melebihi batas 11 pemain
              ...
              // Bagian C: Evaluasi & Catatan Akhir Pertandingan
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () {
                    // Validasi kelayakan data sebelum disimpan
                    if (_selectedCount < 11) {
                      ...
                      // Kumpulkan daftar nama pemain yang terpilih sebagai starting XI
                      ...
                      // Mengirimkan event 'save_manual_match' berisi data lengkap ke server backend
