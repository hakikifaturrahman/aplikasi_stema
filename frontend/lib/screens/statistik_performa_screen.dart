/// Halaman Visualisasi Statistik & Performa Pemain STEMA
/// 
/// Halaman ini menampilkan visualisasi grafik radar (Radar Chart) untuk 6 atribut skill teknis pemain,
/// grafik garis tren nilai rating performa (Line Chart) pada 5 pertandingan terakhir,
/// serta ringkasan total sprint & rating rata-rata dari database backend.
...
/// Widget Halaman Statistik: StatistikPerformaScreen
...
class _StatistikPerformaScreenState extends State<StatistikPerformaScreen> {
  // ── State Analisis Statistik ──
  List<dynamic> _players = [];              // Menyimpan data stamina & atribut pemain yang disinkronkan dari server
  List<dynamic> _riwayat = [];              // Menyimpan riwayat pertandingan lengkap dari server Node.js
  String? _selectedPlayerName;              // Nama pemain yang sedang dipilih untuk dianalisis statistiknya
  String _selectedMatchPeriode = 'Semua Data'; // Filter waktu periode pertandingan (Semua, Bulan Ini, Minggu Ini)

  @override
  void initState() {
    super.initState();
    // 1. Inisialisasi koneksi Socket.IO global dan mengirim permintaan sinkronisasi awal
    SocketService().connect();
    SocketService().socket.emit('request_sync');

    // 2. Menerima sinkronisasi data roster & stamina pemain via event 'stamina_sync'
    SocketService().socket.on('stamina_sync', (data) {
      if (mounted) {
        setState(() {
          _players = data;
          if (_selectedPlayerName == null && _players.isNotEmpty) {
            _selectedPlayerName = _players.first['nama']; // Set default nama pemain terpilih pertama
          }
        });
      }
    });

    // 3. Menerima sinkronisasi data histori riwayat match via event 'riwayat_sync'
    SocketService().socket.on('riwayat_sync', (data) {
      if (mounted) {
        setState(() {
          _riwayat = data; // Simpan data riwayat pertandingan
        });
      }
    });
  }

  /// Helper untuk mendapatkan detail profil pemain yang dipilih saat ini
  ...
  /// Properti Helper: Mendapatkan riwayat 5 nilai rating performa pertandingan terakhir pemain terpilih
  /// Digunakan untuk merender grafik garis LineChart
  List<double> get _recentRatings {
    if (_riwayat.isEmpty || _selectedPlayerName == null) return [];
    
    List<double> ratings = [];
    // Melakukan iterasi dari riwayat terbaru untuk mencari data rating pemain terpilih
      ...
      if (match['performaPemain'] != null) {
        for (var pInfo in match['performaPemain']) {
          if (pInfo['nama'] == _selectedPlayerName) {
            ...
            ratings.add(rating);
            break; // Ditemukan untuk match ini, lanjut ke match berikutnya
          }
        }
      }
      if (ratings.length >= 5) break; // Ambil maksimal 5 pertandingan terakhir saja
    }
    
    // Membalik urutan (reverse) agar penyajian grafik runut dari pertandingan terlama ke terbaru
