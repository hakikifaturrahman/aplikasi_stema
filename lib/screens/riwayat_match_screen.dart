...
class MatchRecord {
  ...
  final String lawan;
  final String ?timKandang;
  final String skor;
  final String hasil; // W / D / L
  final int totalSubstitusi;
  final List<Map<String, dynamic>> statistikTim;
  final List<String> rekomendasi;
  final List<Map<String, dynamic>> performaPemain;
...
class _RiwayatMatchScreenState extends State<RiwayatMatchScreen>
  ...
  List<MatchRecord> _matches = dummyMatches;
  ...
  void initState() {
    ...
    // Minta data terbaru ketika layar Riwayat dibuka!
    SocketService().connect();
    SocketService().socket.emit('request_sync');

    // Mendengarkan update Riwayat tersinkronisasi dari Node.js
    SocketService().socket.on('riwayat_sync', (data) {
      if (mounted && data != null) {
        setState(() {
          // Gabungkan Data Server Live + Dummy Lama yang ter-hardcode (agar layar tetap penuh)
          final serverMatches = (data as List).map((item) {
            return MatchRecord(
              ...
              // Pastikan tipe data List terkonversi dengan pas
          ...
          // Data server yang baru disusun di atas Dummy!
