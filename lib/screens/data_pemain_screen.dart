...
class _DataPemainScreenState extends State<DataPemainScreen> {
  ...
  void initState() {
    super.initState();
    // 1. Minta sinkronisasi roster dari Server Node.js
    SocketService().connect();
    SocketService().socket.emit('request_sync');

    // 2. Mendengar Update Roster (menggunakan jalur stamina_sync karena struktur datanya sama)
                 ...
                 (player['pos'] ?? '').toString().toLowerCase().contains(_searchQuery)
                  ...
                  return Padding(
                    ...
                    child: _buildPlayerCard(
                      ...
                      stamina: st,
                      staminaColor: stc,
                      imageBase64: player['foto'], // Fetch photo from DB
                      onEdit: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
